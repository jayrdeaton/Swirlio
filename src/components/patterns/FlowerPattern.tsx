...
// The ripple pool/path/width/dash math is shared with Polygon/Rings/Star — see
// useRipplePatternGeometry's own comment — this component supplies only the one thing that's actually
// Flower-specific: which shape addShape draws into the pool at each ripple's radius.
export function FlowerPattern({ radius, pulse, tightness, sides, reversed, strokeWidth, dashStyle, fixedSpacing, referenceRadius, children }: FlowerPatternProps) {
  const geometry = useRipplePatternGeometry({
    ...
    fixedSpacing,
    referenceRadius,
    // Reuses the same slider as Polygon's side count and Star's point count — the drawer labels it
    // "Petals" while Flower is selected, but it's the same underlying setting either way.
    addShape: (builder, radiusAtProgress) => {
      'worklet'
      builder.addPoly(buildFlowerPoints(sides.value, radiusAtProgress), true)
    }
  })
