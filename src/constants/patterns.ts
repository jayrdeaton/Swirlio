export type PatternType = 'flower' | 'heart' | 'polygon' | 'rings' | 'spiral' | 'star' | 'starburst'

// Star, Polygon, and Flower come last (not alongside Rings/Heart) so the drawer's two-per-row split
// puts them together in their own pairing — they're the three patterns that share the
// Sides/Points/Petals slider. Heart sits with Rings/Starburst in the other group for the same reason:
// it has no side count of its own either (see hasPolygonSides below).
export const PATTERN_ORDER: PatternType[] = ['spiral', 'rings', 'heart', 'starburst', 'star', 'polygon', 'flower']
...
// whether the pattern underneath is ripple-based. Rings, Polygon, Star, Flower, and Heart are ripples
